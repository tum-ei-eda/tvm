...
Map<Expr, Array<IntegerArray> > GraphPlanMemory(const Function& func) {
  ...
  Map<Expr, Array<IntegerArray>> out = StorageAllocator().Plan(func);
  for (auto p : out) {
    // Append array for offsets.
    std::vector<Integer> offsets(p.second[0].size());
    std::fill(offsets.begin(), offsets.end(), 0);
    out.Set(p.first, Array<IntegerArray>({p.second[0], p.second[1], p.second[2], offsets}));
  }
  return out;
}
